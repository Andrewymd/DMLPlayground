...
import numpy as np
from models.simplemodels import ScaleLayer
...
class DistanceWeightedSampling(HybridBlock):
    ...
    def hybrid_forward(self, F, x, y):
        ...
        y = y.expand_dims(1)
        x_shape = x.shape_array()
        dim = x_shape.pick(F.ones((1,))).astype('float32')

        distance = pairwise_distance(F, x, squared=False)  # diagonal is 0
        distance = F.diag(F.diag(F.ones_like(distance))) + distance  # diagonal is 1

        # Cut off to avoid high variance.
        distance = F.maximum(distance, self.cutoff)

        # Subtract max(log(distance)) for stability.
        log_weights = F.broadcast_mul(2.0 - dim, F.log(distance)) - F.broadcast_mul(((dim - 3.0) / 2), F.log(1.0 - 0.25 * (distance ** 2.0)))
        weights = F.exp(F.broadcast_sub(log_weights, F.max(log_weights)))

        # Sample only negative examples by setting weights of
        # the same-class examples to 0.
        mask = F.ones_like(weights) - F.broadcast_equal(y, y.transpose())

        weights = weights * mask * (distance < self.nonzero_loss_cutoff)
        weights = F.broadcast_div(weights, F.sum(weights, axis=1, keepdims=True))

        negative_indices = F.sample_multinomial(weights, (k-1,))  # Bx(k-1)

        anchor_indices = F.arange(start=0, stop=self._n, repeat=(k-1))
        ...
        for i in range(self._n):
            ...
            if i in range(start, stop):
                if i == start:
                    r = F.arange(start=start+1, stop=stop)
                elif i == stop - 1:
                    r = F.arange(start=start, stop=stop-1)
                else:
                    r1 = F.arange(start=start, stop=i)
                    r2 = F.arange(start=i+1, stop=stop)
                    r = F.concat(r1, r2, dim=0)
            else:
                r = F.arange(start=start, stop=stop)
            positive_indices.append(r)
        positive_indices = F.concat(*positive_indices, dim=0)

        return anchor_indices, F.take(x, anchor_indices), F.take(x, positive_indices), F.take(x, F.reshape(negative_indices, (-1,)))
...
class MarginNet(HybridBlock):
    ...
    """
    def __init__(self, base_net, emb_dim, **kwargs):
        super(MarginNet, self).__init__(**kwargs)

        with self.name_scope():
            self.base_net = base_net
            self.encoder = nn.HybridSequential()
            self.encoder.add(nn.Dense(emb_dim))
    ...
    def init(self, initializer=mx.init.Uniform(), init_basenet=True, ctx=mx.cpu()):
        if init_basenet:
            self.initialize(initializer, ctx=ctx)
        else:
            self.encoder.initialize(initializer, ctx=ctx)
...
class MarginLoss(gluon.loss.Loss):
    ...
    Outputs:
        - Loss.
    """
    def __init__(self, batch_size, batch_k, beta, margin=0.2, nu=0.0, train_beta=False, weight=None, batch_axis=0, **kwargs):
        super(MarginLoss, self).__init__(weight, batch_axis, **kwargs)
        self._margin = margin
        self._nu = nu
        self._train_beta = train_beta
        self._batch_k = batch_k
        self._sampler = DistanceWeightedSampling(batch_size, batch_k=batch_k, prefix='sampler')
        self._beta = beta

    def hybrid_forward(self, F, x, y):
        # Do sampling
        anchor_indices, anchors, positives, negatives = self._sampler(x, y)

        if self._train_beta:
            # Jointly train class-specific beta.
            beta = self._beta(anchor_indices).squeeze()
