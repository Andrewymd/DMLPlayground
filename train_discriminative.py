...
def parse_args():
    parser = TrainingParser(description='Distance metric learning using discriminative loss',
                            default_logfile='train_discriminative.log',
                            default_model_prefix='discriminative_model')
...
def train(net, opt, train_dataloader, val_dataloader, context, run_id):
    ...
    L = DiscriminativeLoss(train_dataloader._dataset.num_classes(), len(train_dataloader._dataset))
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        for i, batch in p_bar:
            ...
            negative_labels = mx.gluon.utils.split_and_load(batch[2], ctx_list=context, batch_axis=0, even_split=False)
...
def train_discriminative(opt):
    ...
    net = EmbeddingNet(feature_net, [opt.embed_dim, train_dataset.num_classes()], normalize=True, dropout=True)
...
if __name__ == '__main__':
    train_discriminative(parse_args())
